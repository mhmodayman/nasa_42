...
#include "gmseckit.h"
/**********************************************************************/
void ReadFromGmsec(GMSEC_ConnectionMgr ConnMgr,GMSEC_Status status, long EchoEnabled)
{
      ...
      double Second;
      GsMsg = connectionManagerReceive(ConnMgr,GMSEC_WAIT_FOREVER,status);
      CheckGmsecStatus(status);
      Field = messageGetField(GsMsg,"MSG-TEXT",status);
      CheckGmsecStatus(status);
      strcpy(Msg,stringFieldGetValue(Field,status));
      CheckGmsecStatus(status);
      ...
      while(!Done) {
         ...
         messageDestroy(&GsMsg);
      }

      /* Acknowledge receipt */
      GmsecSend("GMSEC.42.RX.MSG.LOG",AckMsg,ConnMgr,status);
